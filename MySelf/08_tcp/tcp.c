...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(192, 168, 199, 180);
...
{
    //mempool --> mbuf
    const unsigned total_len = length + 42;   //eth(14) ip(20) udp(8) payload
...
static struct localhost *get_hostinfo_fromip_port(uint32_t dip, u_int16_t port, uint8_t proto)
{
    struct localhost *host;
...
static int udp_process(struct rte_mbuf *udpmbuf)
{
    ...
    struct in_addr addr;
    addr.s_addr = iphdr->src_addr;
    printf("udp_process ---> src: %s:%d, ", inet_ntoa(addr), ntohs(udphdr->src_port));       
...
static int udp_out(struct rte_mempool *mbuf_pool)
{
    ...
    for(host=lhost; host != NULL; host=host->next)
    {
        ...
        if(dstmac == NULL)
        {
            ...
            struct inout_ring *ring = ringInstance();
            rte_ring_enqueue_burst(ring->out, (void **)&arpbuf, 1, NULL);
...
static ssize_t nrecvfrom(int sockfd, void *buf, size_t len, __attribute__((unused)) int flags, struct sockaddr *src_addr, 
                       ...
                       const struct sockaddr *dest_addr, __attribute__((unused)) socklen_t addrlen)
{
    ...
    struct offload *ol = rte_malloc("off=load", sizeof(struct offload), 0);
    if(ol == NULL)
    {
        return -1;
    }
...
static int udp_server_entry(__attribute__((unused)) void *arg)
{
    ...
    localaddr.sin_addr.s_addr = inet_addr("192.168.199.180");
...
int main(int argc, char *argv[])
{
    ...
    {
        rte_exit(EXIT_FAILURE, "Not Supported eth found\n");
    }
